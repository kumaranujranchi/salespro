...
begin
  ...
  -- We use separate blocks to fail gracefully if tables/columns are missing
  ...
  -- Customers 
  -- Fix for FK violation: "customers_created_by_fkey"
  -- We attempt to delete by tenant_id first
  BEGIN EXECUTE 'DELETE FROM public.customers WHERE tenant_id = $1' USING target_tenant_id; EXCEPTION WHEN others THEN NULL; END;
  -- We ALSO attempt to delete by created_by (orphaned rows handling)
  ...
  -- 4. Unlink and Delete Departments
  ...
  -- This is where it was failing if customers still existed
